...
use App\Models\User;
use App\Notifications\CandidateSignupEmail;
...
trait CandidateTrait
{
    ...
    public function formStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('cv')) {
                $cv_upload = $request->cv;
            }
            if ($request->hasFile('profile_image')) {
                $profile_upload = $request->profile_image;
            }
            ...
            if ($data->fails()) {
                ...
                $generatePassword = Str::random(6);
                $encrpyPassword = bcrypt($generatePassword);
                ...
                if (@$request->current_region) {
                    $in->current_region = @$request->current_region;
                }
                ...
                if ($cv_upload) {
                    $in->cv = uploadFile($cv_upload, 'uploads/cv') ?? null;
                }
                if ($profile_upload) {
                    $in->profile_image = uploadFile($profile_upload, 'uploads/profile') ?? null;
                }
                ...
                $user->is_request = login_active();
                $user->is_login = false;

                $user->status = true;
                $user->password = $encrpyPassword;
                $user->save();
                ...
                $in->notify(new CandidateSignupEmail($data));
                if ($in) {
    ...
    /* candDetailsDelete  */
    public function candDetailsDelete(Request $request) /* Candidate deatils update */
    {
        DB::beginTransaction();
        try {
            if (respValid($request)) {
            ...
            $request = decryptData($request['response']); /* Dectrypt  **/
            $in = Candidate::where('uuid', @$request['c_uuid'])->delete();
            if ($in) {
                DB::commit();
                return sendDataHelper('Details deleted.', [], ok());
            } else {
                DB::rollBack();
                return sendError('Something went wrong', [], error());
            }
